...
const ChannelsView = ({ onOpenEq, onOpenComp }) => {
  const { state } = useDsp();
  const phyOuts = state.outputs.filter((o) => o.kind === "out_phy");
  const virtOuts = state.outputs.filter((o) => o.kind === "out_virt");

  return (
    <div className="overflow-x-auto h-full bg-[#0A0A0A]" data-testid="channels-view">
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0A0A0A] text-white" data-testid="dsp-shell">
      ...
      <main className="grow overflow-hidden">
        {tab === "channels" && (
          <ChannelsView onOpenEq={(id) => setEqOutId(id)} onOpenComp={(id) => setCompOutId(id)} />
        )}
