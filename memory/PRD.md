...
- ✅ **Logical / Physical toggle** in `SignalFlowDiagram` + `ChannelMapPrint` — small two-button group in the print toolbar (screen only, hidden in print). **Logical** shows only routed channels (clean live-show view). **Physical** shows every declared 16+16 (or 8+8) channel for full-system documentation. Caption at bottom of diagram updates accordingly ("32 inputs → 32 outputs" vs "2 active inputs → 2 active outputs").
- ✅ **E2E browser tested**: share URL (1.1 KB) roundtrips state across localStorage wipe + URL nav; OUT5 (untouched) hydrates correctly to default while OUT1 keeps its custom name & mute. EQ handle drag updates band 3 from `freq=1000/gain=0` → `freq=1353/gain=14.4`. Physical toggle expands diagram from 2 ↔ 32 channels.
